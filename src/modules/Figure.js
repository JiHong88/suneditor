...
Figure.prototype = {
	...
	open: function (target, nonResizing, __fileManagerInfo) {
		...
		const l = offset.left - (this.options.iframe ? frameOffset.left + (eventWysiwyg.scrollX || eventWysiwyg.scrollLeft || 0) : 0) - this.editor.frameContext.get('wysiwygFrame').scrollLeft;
...
function ContainerResizingOff() {
	...
	if (!this.isVertical && !/%$/.test(w)) {
		const limit = this.editor.frameContext.get('wysiwygFrame').clientWidth - this.editor._editorPadding.left + this.editor._editorPadding.right - 2;
