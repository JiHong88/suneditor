...
class Selection_ extends CoreInjector {
	...
	get() {
		...
		if (typeof this.editor.shadowRoot?.getSelection === 'function') {
			selection = this.editor.shadowRoot.getSelection();
		} else {
	 ...
	 */
	getRange() {
		const range = this.status._range || this.#createDefaultRange();
...
Ï	 * @description Set "range" and "selection" info.
	 ...
	 */
	__init() {
		this.#hasScrollParents = this.eventManager?.scrollparents.length > 0;
