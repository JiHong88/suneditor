...
class UIManager {
	...
	constructor(editor) {
		 ...
		 * @type {import('../../modules/contract/Modal').default}
		 */
		this.opendModal = null;

		/**
		 * @description Currently open "Browser" instance
		 * @type {import('../../modules/contract/Browser').default}
		 */
	...
	setDir(dir) {
		...
		try {
			...
			if (this.#editor.isBalloon) this.#toolbar._showBalloon();
			else if (this.#editor.isSubBalloon) this.#subToolbar._showBalloon();
		} catch (e) {
			this.#options.set('_rtl', !rtl);
	...
	preventToolbarHide(allow) {
		this._notHideToolbar = allow;
	}

	get isPreventToolbarHide() {
		return this._notHideToolbar;
	}
	...
	#activeDirBtn(rtl) {
		...
		this.#commandDispatcher.applyTargets('dir', (e) => {
			dom.utils.changeTxt(e.querySelector('.se-tooltip-text'), this.#editor.lang[rtl ? 'dir_ltr' : 'dir_rtl']);
	...
	_toggleCodeViewButtons(isCodeView) {
		dom.utils.setDisabled(this._codeViewDisabledButtons, isCodeView);
	}
	/**
	 * @internal
	...
	_toggleControllerButtons(isOpen) {
		dom.utils.setDisabled(this._controllerOnDisabledButtons, isOpen);
	}
	...
	isButtonDisabled(button) {
		if (this.#frameContext.get('isReadOnly') && dom.utils.arrayIncludes(this._controllerOnDisabledButtons, button)) {
			return true;
		}
		return false;
	}
	...
	_emitResizeEvent(fc, h, resizeObserverEntry) {
		...
		if (fc.get('_editorHeight') !== h) {
			this.#editor.triggerEvent('onResizeEditor', { height: h, prevHeight: fc.get('_editorHeight'), frameContext: fc, observerEntry: resizeObserverEntry });
