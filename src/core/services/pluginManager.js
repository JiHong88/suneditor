...
import { ACTIVE_EVENT_COMMANDS } from './actives';
...
class PluginManager {
	...
	/**
	 * @constructor
	 * @param {SunEditor.Core} editor - The editor instance (for accessing icons, lang, etc.)
	...
	/**
	 * @description Returns the component checkers array.
	 * @returns {Array<ComponentChecker>}
	 */
	get componentCheckers() {
		return this.#componentCheckers;
	}
	...
	register(pluginName, targets, pluginOptions) {
		...
		if (!plugin) {
			...
			plugin = this.#plugins[pluginName] = new this.#plugins[pluginName](this.#editor, pluginOptions || {});
		}

		if (targets) {
			const { icons, lang } = this.#editor;
					...
					return {
						...
						launcher: {
							destroy: (target) => {
								...
								// focus
								this.#editor.focusManager.focusEdge(focusEl);
								this.#editor.history.push(false);
							},
						},
					};
				}.bind(this),
