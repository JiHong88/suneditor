...
2. 커밋 요청으로 승인된 작업의 파일만 `git add`로 스테이징한다. 기존 staged diff를 먼저 읽고, 다른 작업의 staged 변경을 임의로 포함하거나 unstage하지 않는다. 일부 파일에 작업이 섞여 있으면 범위를 분리해 확인한다.
3. `git diff --staged` 분석 → 아래 포맷/규칙에 따라 커밋 메시지 생성
4. **[승인 게이트 — 반드시 지킬 것]** 생성한 메시지 초안을 채팅에 코드블록으로 그대로 보여주고
   **"이대로 커밋할까요? 수정할 곳 있으면 알려주세요"** 라고 물은 뒤 **거기서 멈춘다.**
   - 이 단계에서는 **절대 `git commit`을 실행하지 않는다.** 사용자의 승인 응답을 기다린다.
   - 사용자가 수정을 요청하면 메시지를 고쳐 다시 초안을 보여주고 재승인을 받는다.
   - 사용자가 승인("ok", "ㄱㄱ", "커밋" 등)하면 그때 `git commit`을 실행한다.
   - 예외: 사용자가 처음부터 "바로 커밋", "확인 없이 커밋" 등 명시적으로 게이트 생략을 요청한 경우에만 곧바로 커밋한다.
