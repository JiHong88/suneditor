...
test.describe('Table Plugin E2E Tests', () => {
  ...
  test('should create multiple tables', async ({ page }) => {
    // Create first table
    await createTableWithPicker(page, 2, 2);
    await page.waitForTimeout(300);

    // Move cursor outside table (press Down arrow multiple times and Enter)
    ...
    // Verify two tables exist
    const tables = await page.locator('.se-wrapper-wysiwyg table').count();
  ...
  test('should select cell on click', async ({ page }) => {
    ...
    const secondCell = page.locator('.se-wrapper-wysiwyg table td').nth(1);
    await secondCell.click();
    await page.waitForTimeout(300);

    // Check if cell is focused/selected (has selection or is in focus)
    const isFocused = await page.evaluate(() => {
      const activeEl = document.activeElement;
      return activeEl && activeEl.closest('td') !== null;
    });
  ...
  test('should have table plugin methods accessible', async ({ page }) => {
    const hasTablePlugin = await page.evaluate(() => {
      return window.editor_root &&
             window.editor_root.plugins &&
             window.editor_root.plugins.table !== undefined;
    });
